...
export function generateJS(program: AST.Program): string {
    const exports: string[] = []
    for (const node of program.body) {
        if (node.kind === "FnDecl" && node.annotation?.name !== "native") {
            exports.push(node.name)
        }
    }

    const exportList = exports
        .map(n => `    ${n}: (...args) => instance.exports.${n}?.(...args)`)
        .join(",\n")

    return `// Flow app — generato automaticamente
const Flow = (() => {
